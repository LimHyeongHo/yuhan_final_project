...
// [UI-RQ-003] showPurchases=false면 "진행한 공동구매" 섹션 제외 (개설한 공동구매 관리 탭과 중복)
const SellerProfileContent = ({ email, showPurchases = true }) => {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-[32px] p-8 border border-gray-200 shadow-sm flex flex-col items-center justify-center gap-3 py-20">
        <Loader2 size={28} className="text-gray-300 animate-spin" />
        <span className="text-sm font-bold text-gray-400">판매자 정보를 불러오는 중입니다...</span>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-[32px] p-8 border border-gray-200 shadow-sm flex flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <div className="flex items-center gap-2 mt-1">
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-1.5 text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition px-4 py-2.5 rounded-xl"
          >
            <Home size={14} /> 홈으로
          </button>
          <button
            onClick={() => navigate('/buyer/products')}
            className="flex items-center gap-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 transition px-4 py-2.5 rounded-xl"
          >
            <Search size={14} /> 공구 찾기
          </button>
        </div>
      </div>
    );
